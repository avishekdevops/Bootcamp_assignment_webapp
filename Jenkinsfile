...
try{
    stage('Git checkout'){
        git credentialsId: 'amodak', url: 'https://github.com/avishekdevops/Bootcamp_assignment_webapp.git'
    }
    stage('Build & Test'){
    ...
    stage('Installation of Docker in new provisioned EC2 Instance'){
        ...
        def dockerrun = 'sudo docker run -p 9090:8080 -d amodak/webapp:1.0'
